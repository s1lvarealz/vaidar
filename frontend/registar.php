    ...
    <div class="signup-container">
        ...
        <form id="signupForm" action="../backend/registar.php" method="POST">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <span class="info-icon"
                        data-tooltip="O seu nome de exibição deve ter entre 3 e 16 caracteres, e pode conter letras e números, além de traços, pontos e sublinhados (sem espaços).">
                    ...
                    <p class="error-text" id="nickError"></p>
                </div>
            ...
            <div class="form-group">
                ...
                <p class="login-link">Já está registado? <a href="login.php">Inicie sessão</a></p>
            </div>
        </form>
    </div>

    <footer>
        <p>&copy; 2025 ORANGE. Todos os direitos reservados.</p>
    </footer>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            async function validateNick() {
                ...
                if (/\s/.test(nickValue)) {
                    nickError.textContent = "Nome de utilizador não pode conter espaços.";
                    nickError.style.color = 'red';
                    nickInput.classList.add('incorreto');
                    return;
                }
                ...
                if (nickValue.length < 3 || nickValue.length > 16) {
                    nickError.textContent = "Nome de utilizador deve ter entre 3 e 16 caracteres.";
                    nickError.style.color = 'red';
                    nickInput.classList.add('incorreto');
                    return;
                }

                // Verifica se já existe (mantendo a verificação original)
                const response = await fetch(`../backend/check_nick.php?nick=${nickValue}`);
                const result = await response.text();
                if (result === 'exist') {
                    nickError.textContent = 'Nome de utilizador já está em uso.';
                    nickError.style.color = 'red';
                    nickInput.classList.add('incorreto');
                } else {
                    nickError.style.color = 'transparent';
